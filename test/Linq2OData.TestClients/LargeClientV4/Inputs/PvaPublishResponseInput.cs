// <auto-generated>
//     This code was generated by Linq2OData, https://github.com/joadan/Linq2OData/ 
//     Changes to this sharedFile may cause incorrect behavior and will be lost if the code is regenerated.
// </auto-generated>

#nullable enable

using Linq2OData.Core;

namespace Large.Microsoft.Dynamics.CRM;

/// <summary>
/// OdataName: PvaPublishResponse
/// </summary>
public partial class PvaPublishResponseInput : ODataInputBase
{
	// IsEnumType: False
    public string? PublishedBotContentId 
	{
		get => GetValue<string?>("PublishedBotContentId");
		set => SetValue("PublishedBotContentId", value);
	}

    //Navigations

}
